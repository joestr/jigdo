...
/*[RA]void* MD5Sum::md5_read_ctx(const md5_ctx *ctx, void* resbuf) */
/*[RA]{ */
/*[RA]  ((uint32 *) resbuf)[0] = SWAP (ctx->A); */
/*[RA]  ((uint32 *) resbuf)[1] = SWAP (ctx->B); */
/*[RA]  ((uint32 *) resbuf)[2] = SWAP (ctx->C); */
/*[RA]  ((uint32 *) resbuf)[3] = SWAP (ctx->D); */
/*[RA] */
/*[RA]  return resbuf; */
/*[RA]} */
byte* MD5Sum::md5_read_ctx(const md5_ctx *ctx, byte* resbuf) {
  byte* r = resbuf;
  uint32 x = SWAP(ctx->A);
  *r++ = static_cast<byte>((x) & 0xff);
  *r++ = static_cast<byte>((x >> 8) & 0xff);
  *r++ = static_cast<byte>((x >> 16) & 0xff);
  *r++ = static_cast<byte>((x >> 24) & 0xff);
  x = SWAP(ctx->B);
  *r++ = static_cast<byte>((x) & 0xff);
  *r++ = static_cast<byte>((x >> 8) & 0xff);
  *r++ = static_cast<byte>((x >> 16) & 0xff);
...
/*[RA] #define CYCLIC(w, s) (w = (w << s) | (w >> (32 - s))) */
#define CYCLIC(w, s) (w = (w << s) | ((w & 0xffffffff) >> (32 - s)))
