/* $Id: configfile.cc,v 1.15 2002/02/13 00:36:16 richard Exp $ -*- C++ -*-
  __   _
...
void ConfigFile::rescan(bool printErrors) {
  ...
  for (iterator i = begin(), e = end(); i != e; ++i) {
    ...
    if (printErrors) {
      ...
      if (*x != ']') {
        string err = subst(_("Section name contains invalid character `%1'"),
                           *x);
        reporter->error(err, thisLine);
        continue;
      }
      ...
      if (s2 - s1 == 7 && (*s1 == 'C' || *s1 == 'c')) {
        ...
        int i = 1;
        do { if (*++s1 != omment[i]) break; ++i; } while (i < 7);
        inComment = (i == 7);
      }
    }
...
ConfigFile::iterator& ConfigFile::iterator::nextSection(
  ...
  while (!p->text.empty()) { // i.e. while end() not reached
    ...
    if (s == send // End of sectName; now only ']' and whitespace may follow
      ...
      return *this; // Found matching [section] line
    }
